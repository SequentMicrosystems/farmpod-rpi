...
int doRtdTempRead(int argc, char *argv[])
{
	...
	uint8_t buf[4];
	if (OK
		!= i2cMem8Read(dev, I2C_MEM_RTD_TEMP1 + RTD_TEMP_DATA_SIZE * (ch - 1),
			buf, RTD_TEMP_DATA_SIZE))
	{
...
int doRtdResRead(int argc, char *argv[])
{
	...
	uint8_t buf[4];
	if (OK
		!= i2cMem8Read(dev, I2C_MEM_RTD_RES1 + RTD_RES_DATA_SIZE * (ch - 1), buf,
			RTD_RES_DATA_SIZE))
	{
...
int doRtdResCal(int argc, char *argv[])
{
	...
	{
		if (OK != calibReset(dev, CAL_RTD_RES_CH1 + (ch - 1)))
		{
	...
	if (OK != calibSet(dev, CAL_RTD_RES_CH1 + (ch - 1), val))
	{
		printf("Failed to read rtd resistance");
		return ERROR ;
	}
